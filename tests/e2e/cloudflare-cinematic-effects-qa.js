...
const MEDIA_EXTENSION_PATTERN =
  /\.(gif|jpe?g|mp3|mp4|ogg|png|webm|webp|wav)(\?|$)/i;
...
const RIP_COMPOSITION_SELECTOR =
...
function screenshotPathForViewport(basePath, viewportName, index) {
  ...
  return basePath.replace(
    ...
    (fileName, extension) => {
      ...
      const stem =
        uniqueStem.length <= SEMANTIC_FILE_STEM_MAX_LENGTH
          ? uniqueStem
  ...
  return page.evaluate(
    ({
      ...
      const usableViewportWidth = Math.max(
        0,
        viewportWidth - browserChromeLeftInset - browserChromeRightInset,
      );
      const usableViewportHeight = Math.max(
        0,
        viewportHeight - browserChromeTopInset - browserChromeBottomInset,
      );
      ...
      const pxCustomProperty = (propertyName) => {
        ...
        const parsed = Number.parseFloat(
          stageStyle.getPropertyValue(propertyName),
        );
      ...
      const cssBrowserChromeRightInset = hasCssVisibleViewport
        ? Math.max(
            0,
            viewportWidth - cssBrowserChromeLeftInset - cssVisibleViewportWidth,
          )
        : null;
      ...
      return {
        ...
        media: [
          ...(overlay?.querySelectorAll("img[data-cinematic-media]") || []),
        ].map((item) => ({
        ...
        mediaLayers: [
          ...(overlay?.querySelectorAll(mediaLayerSelector) || []),
        ].map((item) => ({
    ...
    (targetSelector) =>
      [
        ...document.querySelectorAll(
          `${targetSelector} img[data-cinematic-media]`,
        ),
      ].every((image) => {
...
function assertFullViewport(snapshot, label) {
  ...
  assert(
    snapshot.rect.width >= snapshot.viewport.width * 0.96,
    `${label}: largura não cobre viewport.`,
  );
  assert(
    snapshot.rect.height >= snapshot.viewport.height * 0.96,
    `${label}: altura não cobre viewport.`,
  );
}
...
function assertBoardAnchoring(snapshot, label) {
  ...
  assert(
    snapshot.canvasRect,
    `${label}: canvas ausente para comparar ancoragem.`,
  );
  assert(
    snapshot.contentRect,
    `${label}: conteúdo textual sem retângulo visual.`,
  );
  assert(
    Math.abs(snapshot.stageRect.x - snapshot.canvasRect.x) <=
      MAX_STAGE_OFFSET_PX,
    `${label}: palco desalinhado no eixo X.`,
  );
  assert(
    Math.abs(snapshot.stageRect.y - snapshot.canvasRect.y) <=
      MAX_STAGE_OFFSET_PX,
    `${label}: palco desalinhado no eixo Y.`,
  );
  assert(
    Math.abs(snapshot.stageRect.width - snapshot.canvasRect.width) <=
      MAX_STAGE_OFFSET_PX,
    `${label}: largura do palco difere do canvas.`,
  );
  assert(
    Math.abs(snapshot.stageRect.height - snapshot.canvasRect.height) <=
      MAX_STAGE_OFFSET_PX,
    `${label}: altura do palco difere do canvas.`,
  );
  assert(
    Math.abs(snapshot.contentRect.centerX - snapshot.canvasRect.centerX) <=
      MAX_CONTENT_CENTER_OFFSET_PX,
    `${label}: conteúdo textual descentralizado no eixo X.`,
  );
  assert(
    Math.abs(snapshot.contentRect.centerY - snapshot.canvasRect.centerY) <=
      MAX_CONTENT_CENTER_OFFSET_PX,
    `${label}: conteúdo textual descentralizado no eixo Y.`,
  );

  if (snapshot.compositionRect) {
    assert(
      Math.abs(
        snapshot.compositionRect.centerX - snapshot.canvasRect.centerX,
      ) <= MAX_CONTENT_CENTER_OFFSET_PX,
      `${label}: composição visual descentralizada no eixo X.`,
    );
    assert(
      Math.abs(
        snapshot.compositionRect.centerY - snapshot.canvasRect.centerY,
      ) <= MAX_CONTENT_CENTER_OFFSET_PX,
      `${label}: composição visual descentralizada no eixo Y.`,
    );
    assert(
      snapshot.compositionRect.x >=
        snapshot.canvasRect.x - MAX_MEDIA_OVERFLOW_PX,
      `${label}: composição visual saiu pela esquerda do canvas.`,
    );
    assert(
      snapshot.compositionRect.y >=
        snapshot.canvasRect.y - MAX_MEDIA_OVERFLOW_PX,
      `${label}: composição visual saiu pelo topo do canvas.`,
    );
    assert(
      snapshot.compositionRect.right <=
        snapshot.canvasRect.right + MAX_MEDIA_OVERFLOW_PX,
      `${label}: composição visual saiu pela direita do canvas.`,
    );
    assert(
      snapshot.compositionRect.bottom <=
        snapshot.canvasRect.bottom + MAX_MEDIA_OVERFLOW_PX,
      `${label}: composição visual saiu pelo rodapé do canvas.`,
    );
  }
  ...
  for (const item of snapshot.media) {
    ...
    assert(
      item.rect.width > 0,
      `${label}: mídia sem largura visual ${item.id}.`,
    );
    assert(
      item.rect.height > 0,
      `${label}: mídia sem altura visual ${item.id}.`,
    );
    assert(
      Math.abs(item.rect.centerX - snapshot.canvasRect.centerX) <=
        MAX_MEDIA_CENTER_OFFSET_PX,
      `${label}: mídia ${item.id} descentralizada no eixo X.`,
    );
    assert(
      Math.abs(item.rect.centerY - snapshot.canvasRect.centerY) <=
        MAX_MEDIA_CENTER_OFFSET_PX,
      `${label}: mídia ${item.id} descentralizada no eixo Y.`,
    );
    assert(
      item.rect.x >= snapshot.canvasRect.x - MAX_MEDIA_OVERFLOW_PX,
      `${label}: mídia ${item.id} saiu pela esquerda do canvas.`,
    );
    assert(
      item.rect.y >= snapshot.canvasRect.y - MAX_MEDIA_OVERFLOW_PX,
      `${label}: mídia ${item.id} saiu pelo topo do canvas.`,
    );
    assert(
      item.rect.right <= snapshot.canvasRect.right + MAX_MEDIA_OVERFLOW_PX,
      `${label}: mídia ${item.id} saiu pela direita do canvas.`,
    );
    assert(
      item.rect.bottom <= snapshot.canvasRect.bottom + MAX_MEDIA_OVERFLOW_PX,
      `${label}: mídia ${item.id} saiu pelo rodapé do canvas.`,
    );
  }
}
...
function assertCountdownViewportCentering(snapshot, label) {
  ...
  assert(
    snapshot.countdownCountRect,
    `${label}: número da contagem sem retângulo visual.`,
  );
  assert(
    snapshot.countdownHaloRect,
    `${label}: halo da contagem sem retângulo visual.`,
  );
  assert(
  ...
  assert(
    snapshot.stageRect.width >=
      viewport.width * MIN_RIP_STAGE_VIEWPORT_COVERAGE_RATIO,
    `${label}: palco não cobre a largura da viewport.`,
  );
  assert(
    snapshot.stageRect.height >=
      viewport.height * MIN_RIP_STAGE_VIEWPORT_COVERAGE_RATIO,
    `${label}: palco não cobre a altura da viewport.`,
  );
  assert(
    Math.abs(snapshot.stageRect.centerX - viewport.centerX) <=
      MAX_COUNTDOWN_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: palco fora do centro X da viewport.`,
  );
  assert(
    Math.abs(snapshot.stageRect.centerY - viewport.centerY) <=
      MAX_COUNTDOWN_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: palco fora do centro Y da viewport.`,
  );
  assert(
    Math.abs(snapshot.compositionRect.centerX - viewport.centerX) <=
      MAX_COUNTDOWN_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: composição fora do centro X da viewport.`,
  );
  assert(
    Math.abs(snapshot.compositionRect.centerY - viewport.centerY) <=
      MAX_COUNTDOWN_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: composição fora do centro Y da viewport.`,
  );
  assert(
    Math.abs(snapshot.contentRect.centerX - viewport.centerX) <=
      MAX_COUNTDOWN_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: conteúdo fora do centro X da viewport.`,
  );
  assert(
    Math.abs(snapshot.contentRect.centerY - viewport.centerY) <=
      MAX_COUNTDOWN_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: conteúdo fora do centro Y da viewport.`,
  );
  assert(
    Math.abs(snapshot.countdownCountRect.centerX - viewport.centerX) <=
      MAX_MEDIA_CENTER_OFFSET_PX,
    `${label}: número fora do centro X da área visível.`,
  );
  assert(
    Math.abs(snapshot.countdownCountRect.centerY - viewport.centerY) <=
      MAX_MEDIA_CENTER_OFFSET_PX,
    `${label}: número fora do centro Y da área visível.`,
  );
  assert(
    Math.abs(snapshot.countdownHaloRect.centerX - viewport.centerX) <=
      MAX_MEDIA_CENTER_OFFSET_PX,
    `${label}: halo fora do centro X da área visível.`,
  );
  assert(
    Math.abs(snapshot.countdownHaloRect.centerY - viewport.centerY) <=
      MAX_MEDIA_CENTER_OFFSET_PX,
    `${label}: halo fora do centro Y da área visível.`,
  );
  assertRectInsideVisualViewport(
    snapshot.compositionRect,
    viewport,
    `${label}: composição countdown`,
  );
  assertRectInsideVisualViewport(
    snapshot.countdownHaloRect,
    viewport,
    `${label}: halo countdown`,
  );

  for (const item of snapshot.media) {
    assert(item.rect, `${label}: mídia sem retângulo visual ${item.id}.`);
    assert(
      Math.abs(item.rect.centerX - viewport.centerX) <=
        MAX_MEDIA_CENTER_OFFSET_PX,
      `${label}: mídia ${item.id} fora do centro X da viewport.`,
    );
    assert(
      Math.abs(item.rect.centerY - viewport.centerY) <=
        MAX_MEDIA_CENTER_OFFSET_PX,
      `${label}: mídia ${item.id} fora do centro Y da viewport.`,
    );
    assertRectInsideVisualViewport(
      item.rect,
      viewport,
  ...
  for (const item of snapshot.mediaLayers) {
    ...
    assert(
      Math.abs(item.rect.centerX - viewport.centerX) <=
        MAX_MEDIA_CENTER_OFFSET_PX,
      `${label}: camada ${item.id} fora do centro X da viewport.`,
    );
    assert(
      Math.abs(item.rect.centerY - viewport.centerY) <=
        MAX_MEDIA_CENTER_OFFSET_PX,
      `${label}: camada ${item.id} fora do centro Y da viewport.`,
    );
    assertRectInsideVisualViewport(
      item.rect,
      viewport,
      `${label}: camada ${item.id}`,
    );
  }
}
...
function assertCountdownFullScreenViewport(snapshot, viewportName) {
  ...
  assert(
    Math.abs(viewport.x - visualViewport.x) <= MAX_STAGE_OFFSET_PX,
    `${viewportName}: countdown deslocou a tela visível no eixo X.`,
  );
  assert(
    Math.abs(viewport.y - visualViewport.y) <= MAX_STAGE_OFFSET_PX,
    `${viewportName}: countdown deslocou a tela visível no eixo Y.`,
  );
  assert(
    Math.abs(viewport.width - visualViewport.width) <= MAX_STAGE_OFFSET_PX,
    `${viewportName}: largura visível do countdown difere da tela inteira.`,
  );
  assert(
    Math.abs(viewport.height - visualViewport.height) <= MAX_STAGE_OFFSET_PX,
    `${viewportName}: altura visível do countdown difere da tela inteira.`,
  );
  assert(
    viewport.browserChromeTopInset <= MAX_STAGE_OFFSET_PX,
    `${viewportName}: countdown reservou topo automaticamente.`,
  );
  assert(
    viewport.browserChromeBottomInset <= MAX_STAGE_OFFSET_PX,
    `${viewportName}: countdown reservou rodapé automaticamente.`,
  );
  assert(
    viewport.browserChromeLeftInset <= MAX_STAGE_OFFSET_PX,
    `${viewportName}: countdown reservou esquerda automaticamente.`,
  );
  assert(
    viewport.browserChromeRightInset <= MAX_STAGE_OFFSET_PX,
    `${viewportName}: countdown reservou direita automaticamente.`,
  );
}
...
function assertRectInsideVisualViewport(rect, viewport, label) {
  ...
  assert(
    rect.x >= viewport.x + RIP_SAFE_AREA_PX,
    `${label}: saiu pela esquerda da viewport.`,
  );
  assert(
    rect.y >= viewport.y + RIP_SAFE_AREA_PX,
    `${label}: saiu pelo topo da viewport.`,
  );
  assert(
    rect.right <= viewport.right - RIP_SAFE_AREA_PX,
    `${label}: saiu pela direita da viewport.`,
  );
  assert(
    rect.bottom <= viewport.bottom - RIP_SAFE_AREA_PX,
    `${label}: saiu pelo rodapé da viewport.`,
  );
}
...
function assertRipViewportCentering(snapshot, label) {
  ...
  assert(
    snapshot.compositionRect,
    `${label}: composição visual do RIP ausente.`,
  );
  assert(
    snapshot.stageRect.width >=
      viewport.width * MIN_RIP_STAGE_VIEWPORT_COVERAGE_RATIO,
    `${label}: palco do RIP não cobre a largura da viewport.`,
  );
  assert(
    snapshot.stageRect.height >=
      viewport.height * MIN_RIP_STAGE_VIEWPORT_COVERAGE_RATIO,
    `${label}: palco do RIP não cobre a altura da viewport.`,
  );
  assert(
    Math.abs(snapshot.stageRect.centerX - viewport.centerX) <=
      MAX_RIP_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: palco do RIP fora do centro X da viewport.`,
  );
  assert(
    Math.abs(snapshot.stageRect.centerY - viewport.centerY) <=
      MAX_RIP_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: palco do RIP fora do centro Y da viewport.`,
  );
  assert(
    Math.abs(snapshot.compositionRect.centerX - viewport.centerX) <=
      MAX_RIP_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: composição do RIP fora do centro X da viewport.`,
  );
  assert(
    Math.abs(snapshot.compositionRect.centerY - viewport.centerY) <=
      MAX_RIP_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: composição do RIP fora do centro Y da viewport.`,
  );
  assert(
    Math.abs(snapshot.contentRect.centerX - viewport.centerX) <=
      MAX_RIP_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: texto do RIP fora do centro X da viewport.`,
  );
  assert(
    Math.abs(snapshot.contentRect.centerY - viewport.centerY) <=
      MAX_RIP_VIEWPORT_CENTER_OFFSET_PX,
    `${label}: texto do RIP fora do centro Y da viewport.`,
  );
  assertRectInsideVisualViewport(
    snapshot.compositionRect,
    viewport,
    `${label}: composição RIP`,
  );
  assertRectInsideVisualViewport(
    snapshot.titleRect,
    viewport,
...
function assertMedia(snapshot, requiredIds, label) {
  ...
  for (const requiredId of requiredIds) {
    assert(
      mediaIds.includes(requiredId),
      `${label}: mídia ausente ${requiredId}.`,
    );
  }
  for (const item of snapshot.media) {
    assert(
      item.src.startsWith("/assets/visual/vfx/"),
      `${label}: mídia fora de /assets/visual/vfx/: ${item.src}`,
    );
    assert(
      item.ariaHidden === "true",
      `${label}: mídia decorativa sem aria-hidden=true: ${item.id}`,
    );
    assert(
      item.alt === "",
      `${label}: mídia decorativa sem alt vazio: ${item.id}`,
    );
  }
}
...
async function cachedCinematicPaths(page) {
  return page.evaluate(
    async ({ paths, manifestPath, hashSearchParam }) => {
...
async function waitForCachedCinematicPaths(page) {
  ...
  do {
    ...
    const missingPaths = REQUIRED_CINEMATIC_MEDIA_PATHS.filter(
  ...
  try {
    const attachDiagnostics = (targetPage) => {
      targetPage.on("request", (request) => {
        ...
        if (
          MEDIA_EXTENSION_PATTERN.test(requestUrl.pathname) &&
          requestUrl.origin !== parsed.origin
        ) {
    ...
    await page.screenshot({
      path: primaryCountdownConfig.screenshotPath,
      fullPage: true,
    });
    ...
    for (const config of countdownViewportConfigs.slice(1)) {
      ...
      await countdownPage.setViewport(
        puppeteerViewport(viewportByName(config.viewportName)),
      );
      ...
      const secondaryCountdownState = await overlaySnapshot(
        countdownPage,
        COUNTDOWN_SELECTOR,
      );
      ensureParentDirectory(config.screenshotPath);
      await countdownPage.screenshot({
      ...
      const secondaryCountdownDurationMs =
        Date.now() - secondaryCountdownStartedAt;
    ...
    for (const result of countdownResults) {
      ...
      assertCountdownFullScreenViewport(
      ...
      assert(
        result.countdownDurationMs <= COUNTDOWN_MAX_DURATION_MS + 900,
        `${label}: demorou demais: ${result.countdownDurationMs}ms.`,
      );
      assert(
        /3|2|1/.test(result.countdownState.text),
        `${label}: não mostrou 3, 2 ou 1.`,
      );
    }
    assert(
      levelUpState.text.includes("Subindo de nível"),
      "Level-up não informou subida de nível.",
    );
    assert(
      levelUpState.text.includes("Fase 2"),
      "Level-up não informou fase 2.",
    );
    assert(!countdownAfterLevel, "Countdown reapareceu entre fases.");
    for (const id of REQUIRED_LEVEL_AUDIO_IDS) {
      assert(
        levelAudioIds.includes(id),
        `Áudio ausente no level-up/countdown: ${id}`,
      );
    }
    ...
    for (const result of ripResults) {
      ...
      assert(
        result.ripDurationMs <= RIP_MAX_DURATION_MS + RIP_OBSERVATION_BUFFER_MS,
        `${label}: RIP demorou demais: ${result.ripDurationMs}ms.`,
      );
      assert(
        result.ripState.text.includes("RIP"),
        `${label}: RIP não ficou visível.`,
      );
      assert(
        !result.ripAfterTimeoutState.rect,
        `${label}: RIP permaneceu visível após o limite.`,
      );
      for (const id of REQUIRED_RIP_AUDIO_IDS) {
        assert(
          result.ripAudioIds.includes(id),
          `${label}: áudio ausente no RIP: ${id}`,
        );
      }
    }

    assert(
      externalAudioRequests.length === 0,
      `Requests externos de áudio: ${externalAudioRequests.join(", ")}`,
    );
    assert(
      externalMediaRequests.length === 0,
      `Requests externos de mídia: ${externalMediaRequests.join(", ")}`,
    );
    for (const path of REQUIRED_CINEMATIC_MEDIA_PATHS) {
      assert(
        cachedPaths.includes(path),
        `Mídia cinematográfica fora do cache PWA: ${path}`,
      );
    }
    assert(
