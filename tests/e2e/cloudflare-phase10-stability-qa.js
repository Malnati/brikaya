...
const DEFAULT_SCREENSHOT_PATH =
  "tmp/screenshots/cloudflare-phase10-stability.png";
const VIEWPORT = {
  width: 393,
  height: 852,
  deviceScaleFactor: 3,
...
function reportPath() {
  return process.env.BRIKAYA_PHASE10_QA_REPORT || DEFAULT_REPORT_PATH;
}

function screenshotPath() {
  return process.env.BRIKAYA_PHASE10_QA_SCREENSHOT || DEFAULT_SCREENSHOT_PATH;
}
...
async function run() {
  ...
  try {
    ...
    await page.waitForFunction(
      async ({ dbName, storeName, dbVersion, expectedLevel }) => {
        ...
        return events.some(
          (event) =>
            ...
            event.gameState?.level === expectedLevel,
        );
      },
      { timeout: MAX_WAIT_MS },
      {
    ...
    await page.waitForFunction(
      async ({ dbName, storeName, dbVersion }) => {
        ...
        return events.some(
          (event) =>
            event.type === "collision" && event.collisionInfo?.type === "wall",
        );
      },
      { timeout: MAX_WAIT_MS },
      {
    ...
    const wallCollision = events.find(
      (event) =>
        event.type === "collision" && event.collisionInfo?.type === "wall",
    );
    ...
    assert(
      gameStart.gameState.level === EXPECTED_LEVEL,
      `game_start não iniciou na fase ${EXPECTED_LEVEL}.`,
    );
    assert(
      gameStart.gameState.ballsCount === 1,
      "Contador inicial de bolas não é 1.",
    );
    assert(
      gameStart.gameState.speedState?.level === EXPECTED_LEVEL,
      `speedState não está na fase ${EXPECTED_LEVEL}.`,
    );
    assert(wallCollision, "Colisão de parede em fase alta não foi registrada.");
    assert(
      wallCollision.collisionInfo.ballPosition.x >= 0,
      "Colisão registrou bola à esquerda do canvas.",
    );
    assert(
      wallCollision.collisionInfo.ballPosition.x <=
        gameStart.gameState.canvasSize.width,
      "Colisão registrou bola à direita do canvas.",
    );
    assert(
      (byType.game_end || 0) === 0,
      "game_end apareceu durante amostra de estabilidade da fase 11.",
    );
    assert(
      (byType.ball_lost || 0) === 0,
      "ball_lost apareceu durante amostra de estabilidade da fase 11.",
    );
    assert(
    ...
    assert(
      latestEvent?.gameState?.ballsCount > 0,
      "Último estado ficou sem bolas ativas.",
    );
    assert(
      !latestEvent?.gameState?.gameOver,
      "Último estado entrou em game over indevido.",
    );
    assert(
      layout.canvas &&
        layout.canvas.width > MIN_CANVAS_SIZE &&
        layout.canvas.height > MIN_CANVAS_SIZE,
      "Canvas não está visível.",
    );
    assert(
      layout.chips.some((text) => text.includes(`Fase ${EXPECTED_LEVEL}`)),
      `HUD não mostra Fase ${EXPECTED_LEVEL}.`,
    );
    assert(
      consoleProblems.length === 0,
      `Console tem problemas: ${JSON.stringify(consoleProblems)}`,
    );
  } finally {
    await browser.close();
  }
