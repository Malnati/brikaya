...
const CAPTURE_SCREENSHOTS_ENV_KEY = "BRIKAYA_THEME_QA_CAPTURE_SCREENSHOTS";
const USER_DATA_DIR_PREFIX = "brikaya-theme-qa-";
const RELOAD_GUARD_STORAGE_KEY = "brikaya-sw-controller-reload";
const RELOAD_GUARD_VALUE = "pending";
const RELOAD_GUARD_INTERVAL_MS = 100;
const RELOAD_GUARD_DURATION_MS = 8000;
const RESET_FLAG_PREFIX = "brikaya-theme-qa-reset";
  ...
  return page.evaluate(
    ({ minTouchTargetSize, forbiddenSources }) => {
      ...
      return {
        ...
        storedTheme: window.localStorage.getItem("brikaya-theme"),
        storedThemeMode: window.localStorage.getItem("brikaya-theme-mode"),
        ...
        storedImageSet: window.localStorage.getItem("brikaya-image-set"),
        storedFontSet: window.localStorage.getItem("brikaya-font-set"),
...
async function run() {
  ...
  const reportPath = env("BRIKAYA_THEME_QA_REPORT", DEFAULT_REPORT_PATH);
  const captureScreenshots = shouldCaptureScreenshots();
  const screenshotPaths = {
    iphone15: {
      contrast: env(
        "BRIKAYA_THEME_QA_IPHONE15_CONTRAST_SCREENSHOT",
        DEFAULT_IPHONE15_CONTRAST_SCREENSHOT,
      ),
      sunset: env(
        "BRIKAYA_THEME_QA_IPHONE15_SUNSET_SCREENSHOT",
        DEFAULT_IPHONE15_SUNSET_SCREENSHOT,
      ),
      ocean: env(
        "BRIKAYA_THEME_QA_IPHONE15_OCEAN_SCREENSHOT",
        DEFAULT_IPHONE15_OCEAN_SCREENSHOT,
      ),
      ruby: env(
        "BRIKAYA_THEME_QA_IPHONE15_RUBY_SCREENSHOT",
        DEFAULT_IPHONE15_RUBY_SCREENSHOT,
      ),
      metro: env(
        "BRIKAYA_THEME_QA_IPHONE15_METRO_SCREENSHOT",
        DEFAULT_IPHONE15_METRO_SCREENSHOT,
      ),
    },
    desktop: {
      contrast: env(
        "BRIKAYA_THEME_QA_DESKTOP_CONTRAST_SCREENSHOT",
        DEFAULT_DESKTOP_CONTRAST_SCREENSHOT,
      ),
      sunset: env(
        "BRIKAYA_THEME_QA_DESKTOP_SUNSET_SCREENSHOT",
        DEFAULT_DESKTOP_SUNSET_SCREENSHOT,
      ),
      ocean: env(
        "BRIKAYA_THEME_QA_DESKTOP_OCEAN_SCREENSHOT",
        DEFAULT_DESKTOP_OCEAN_SCREENSHOT,
      ),
      ruby: env(
        "BRIKAYA_THEME_QA_DESKTOP_RUBY_SCREENSHOT",
        DEFAULT_DESKTOP_RUBY_SCREENSHOT,
      ),
      metro: env(
        "BRIKAYA_THEME_QA_DESKTOP_METRO_SCREENSHOT",
