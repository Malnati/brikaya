  ...
  return page.evaluate(
    (timeoutMs) =>
      new Promise((resolve) => {
        ...
        const handleControllerChange = () => done(Boolean(navigator.serviceWorker.controller));
...
async function fetchExpectedSvgs(page) {
  return page.evaluate(
    ...
    { expectedPaths: EXPECTED_SVG_PATHS, cacheStatusHeader: ASSET_CACHE_STATUS_HEADER },
  );
}
...
async function readCacheState(page) {
  return page.evaluate(
    async ({ expectedPaths, manifestPath, hashSearchParam }) => {
      const result = { cacheNames: [], cachedExpected: [], missingExpected: [], manifestAssets: 0 };
      ...
      for (const expectedPath of expectedPaths) {
        ...
        const candidateUrls = [new URL(expectedPath, location.origin).toString()];
        if (hash) {
