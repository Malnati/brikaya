...
const DEFAULT_REPORT_PATH =
  "tmp/reports/cloudflare-no-score-reset-after-brick.json";
const DEFAULT_SCREENSHOT_PATH =
  "tmp/screenshots/cloudflare-no-score-reset-after-brick.png";
const VIEWPORT = {
  width: 393,
  height: 852,
  deviceScaleFactor: 3,
...
async function run() {
  ...
  try {
    ...
    const expectedReductionPerBrick = scoreSpeedState
      ? roundSpeedValue(
          (scoreSpeedState.maxSpeed - scoreSpeedState.minSpeed) /
            scoreSpeedState.initialBrickCount,
        )
      : null;
    ...
    assert(
      !restartedToInitialPosition,
      "Bolinha voltou à posição inicial após colisão com tijolo.",
    );
    assert(
