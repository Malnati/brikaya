...
const DEFAULT_SCREENSHOT_PATH =
  "docs/assets/issues/audio-cc0-integration/evidence/evi-audio-cc0-integration-cloudflare-audio-control.png";
const BROWSER_CLOSE_TIMEOUT_MS = 5000;
const AUDIO_EVENT_IDS_PATTERN =
  /export const AUDIO_EVENT_IDS = (\[[\s\S]*?\]) as const;/;
...
const IOS_CHROME_USER_AGENT =
  "Mozilla/5.0 (iPhone; CPU iPhone OS 18_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/126.0.0.0 Mobile/15E148 Safari/604.1";
...
function expectedLatencySensitiveAudioPaths() {
  ...
  assert(
    objectStart >= 0 && objectEnd >= 0,
    "AUDIO_CATALOG sem bloco parseável.",
  );
...
async function runViewportQa(
  ...
  await page.waitForFunction(() => Boolean(window.__brikayaRunAudioTour), {
    timeout: 30000,
  });
  ...
  assert(
    initial.heading.includes("Brikaya"),
    `${config.name}: tela Brikaya não carregou.`,
  );
  ...
  assert(
    ...
    `${config.name}: estado QA inicial não está mudo.`,
  );
  assert(
    initial.audioState.musicMuted === false,
  ...
  assert(
    ...
    `${config.name}: ícone ligado não é ♪.`,
  );
  assert(
    enabled.soundButton?.ariaPressed === "true",
    `${config.name}: aria-pressed ligado inválido.`,
  );
  assert(
    enabled.audioState.unlocked === true,
    `${config.name}: áudio não ficou unlocked.`,
  );
  assert(
    enabled.audioState.muted === false,
    `${config.name}: áudio não ficou ativo.`,
  );
  assert(
    enabled.audioState.contextState === "running",
    `${config.name}: AudioContext não está running.`,
  );
  assert(
    enabled.audioState.lastUnlockResult?.unlocked === true,
    `${config.name}: último unlock não marcou sucesso.`,
  );
  assert(
    enabled.audioState.latencySensitiveEffectsReady === true,
    `${config.name}: efeitos instantâneos não ficaram prontos após ligar som.`,
  );
  const missingLatencySensitivePaths = latencySensitiveAudioPaths.filter(
    (path) => !enabled.audioState.loadedPaths?.includes(path),
  );
  assert(
  ...
  assert(
    ...
    `${config.name}: ícone de música pausada não é ×.`,
  );
  assert(
    musicDisabled.audioState.muted === false,
    `${config.name}: som geral foi mutado ao pausar música.`,
  );
  assert(
    musicDisabled.audioState.musicMuted === true,
  ...
  assert(
    ...
    `${config.name}: ícone de música ativa não é ♫.`,
  );
  assert(
    musicEnabled.audioState.musicMuted === false,
  ...
  assert(
    ...
    `${config.name}: ícone desligado não voltou para ×.`,
  );
  assert(
    disabled.soundButton?.ariaPressed === "false",
    `${config.name}: aria-pressed desligado inválido.`,
  );
  assert(
    disabled.audioState.muted === true,
  ...
  assert(
    reenabled.audioState.unlocked === true,
    `${config.name}: áudio não continuou unlocked no tour.`,
  );
  assert(
    reenabled.audioState.muted === false,
    `${config.name}: áudio não continuou ativo no tour.`,
  );
...
async function run() {
  ...
  const screenshotPath = env(
    "BRIKAYA_AUDIO_QA_SCREENSHOT",
  ...
  try {
    ...
    const emittedIds = new Set(
      viewportResults.flatMap((result) =>
        result.reenabled.audioState.events.map((event) => event.id),
      ),
    );
    const cachedAudioPaths = [
      ...new Set(
        viewportResults.flatMap((result) => result.reenabled.cachedAudioPaths),
      ),
    ].sort();
    const cacheNames = [
      ...new Set(
        viewportResults.flatMap((result) => result.reenabled.cacheNames),
      ),
    ].sort();
    const missingIds = ids.filter((id) => !emittedIds.has(id));
    const requestedAudioPaths = [...new Set(sameOriginAudioRequests)].sort();
    const uncachedRequestedAudioPaths = requestedAudioPaths.filter(
      (path) => !cachedAudioPaths.includes(path),
    );
    const unknownSameOriginAudioRequests = sameOriginAudioRequests.filter(
      (path) => !audioPathSet.has(path),
    );

    assert(
      missingIds.length === 0,
      `IDs de áudio não emitidos: ${missingIds.join(", ")}`,
    );
    assert(
    ...
    assert(
      unknownSameOriginAudioRequests.length === 0,
      `Requests de áudio fora do manifesto: ${unknownSameOriginAudioRequests.join(", ")}`,
    );
    assert(
      cachedAudioPaths.length >= MIN_CACHED_AUDIO_PATHS,
      "Nenhum áudio foi cacheado após primeiro uso.",
    );
    assert(
      uncachedRequestedAudioPaths.length === 0,
      `Áudios usados não encontrados no cache: ${uncachedRequestedAudioPaths.join(", ")}`,
    );
