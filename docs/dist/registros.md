...
| Coreia `ko` | Naver Search Advisor | Opcional gratuito se conta Naver autorizada existir | R$ 0 | Controlador do site com login Naver. | Login; prova de posse; sitemap. | Concluído em 2026-07-04: site verificado por arquivo HTML público oficial e sitemap enviado sem script runtime. |
...
| Naver Search Advisor | Coreano/`ko`. | Concluído: site verificado, arquivo HTML público oficial validado e `sitemap.xml` enviado. | Sem script runtime, sem ads e sem serviço pago; manter arquivo público de verificação enquanto o Naver depender dele. |
