...
function drawBoundarySegments(
  ...
  const shadowBlur = state.reducedEffects ? 0 : undefined;
...
export function drawBallTurretTrampolines(
  ...
  ctx.shadowBlur = reducedEffects
    ? 0
    : Math.max(8, trampoline.thickness * TRAMPOLINE_SHADOW_BLUR_RATIO);
  ...
  ctx.shadowBlur = reducedEffects ? 0 : Math.max(5, trampoline.thickness * 0.34);
