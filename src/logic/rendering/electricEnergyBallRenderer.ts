...
export type EnergyBallColorTheme = "cyan" | "red" | "green";

interface EnergyBallTheme {
  ...
  coreDeep: string;
  shadow: string;
  arcAlt: [number, number, number];
  shell: string;
  shellShadow: string;
}

const ENERGY_BALL_THEMES: Record<EnergyBallColorTheme, EnergyBallTheme> = {
  cyan: {
    haloMid: [125, 249, 255],
    haloOuter: [0, 178, 255],
    coreLight: "#dffcff",
...
export class ElectricEnergyBallRenderer {
  private readonly colorTheme: EnergyBallColorTheme;

  constructor(
    private readonly centerX = 60,
    private readonly centerY = 60,
    private readonly radius = 18,
    colorTheme: EnergyBallColorTheme = "cyan",
  ) {
    this.colorTheme = ENERGY_BALL_THEMES[colorTheme] ? colorTheme : "cyan";
  }

  private get theme(): EnergyBallTheme {
    return ENERGY_BALL_THEMES[this.colorTheme];
  }

  draw(ctx: CanvasRenderingContext2D, now = Date.now(), reducedEffects = false): void {
    const rawTime = reducedEffects ? 0 : now / 1000;
    const pulse = reducedEffects ? 0 : Math.sin(rawTime * PULSE_RATE) * VISUAL_PULSE_RATIO;
    const visualRadius = this.radius * (1 + pulse);

    ctx.save();
    try {
      ctx.translate(this.centerX, this.centerY);
      this.drawOuterHalo(ctx, visualRadius, rawTime, reducedEffects);
      this.drawCore(ctx, visualRadius, reducedEffects);
      this.drawArcs(ctx, visualRadius, rawTime, reducedEffects);
      this.drawShell(ctx, visualRadius, reducedEffects);
    } finally {
      ctx.restore();
    }
  }

  private drawOuterHalo(
    ...
    haloGradient.addColorStop(0.28, `rgba(${theme.haloMid[0]}, ${theme.haloMid[1]}, ${theme.haloMid[2]}, ${0.52 + haloPulse})`);
    haloGradient.addColorStop(0.64, `rgba(${theme.haloOuter[0]}, ${theme.haloOuter[1]}, ${theme.haloOuter[2]}, ${0.2 + haloPulse * 0.5})`);
  ...
  private drawCore(ctx: CanvasRenderingContext2D, visualRadius: number, reducedEffects: boolean): void {
    const theme = this.theme;
  ...
  private drawShell(ctx: CanvasRenderingContext2D, visualRadius: number, reducedEffects: boolean): void {
    const theme = this.theme;
    ctx.strokeStyle = theme.shell;
    ctx.lineWidth = Math.max(MIN_LINE_WIDTH, visualRadius * 0.12);
    ctx.shadowColor = theme.shellShadow;
    ctx.shadowBlur = reducedEffects ? 0 : Math.max(1.5, visualRadius * 0.28);
    ctx.beginPath();
