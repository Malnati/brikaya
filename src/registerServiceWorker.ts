...
export const BRIKAYA_OFFLINE_READY_EVENT = "brikaya-offline-ready";
export const BRIKAYA_UPDATE_PROGRESS_EVENT =
  "brikaya-update-progress";
export const BRIKAYA_UPDATE_INSTALLED_KEY =
  "brikaya-update-installed-version";
export const BRIKAYA_RELOAD_GUARD_KEY =
  "brikaya-sw-controller-reload";
...
export interface BrikayaUpdateProgressDetail {
  stage: string;
  progress: number;
}
...
function getReloadGuard(windowRef: Window) {
  try {
    return windowRef.sessionStorage.getItem(BRIKAYA_RELOAD_GUARD_KEY);
  } catch {
    return null;
  }
}
...
function clearReloadGuard(windowRef: Window) {
  try {
    windowRef.sessionStorage.removeItem(BRIKAYA_RELOAD_GUARD_KEY);
  } catch {}
}
...
function dispatchUpdateProgress(
  windowRef: Window,
  detail: BrikayaUpdateProgressDetail,
) {
  windowRef.dispatchEvent(
    new CustomEvent(BRIKAYA_UPDATE_PROGRESS_EVENT, { detail }),
  );
}
...
function dispatchOfflineReady(windowRef: Window) {
  windowRef.dispatchEvent(new CustomEvent(BRIKAYA_OFFLINE_READY_EVENT));
}
