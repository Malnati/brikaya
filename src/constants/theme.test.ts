...
describe('theme preference', () => {
  ...
  it('ignora tema salvo inválido', () => {
    expect(resolveInitialTheme('store' as any, false)).toBe(THEME_LIGHT);
    expect(resolveInitialTheme('ranking' as any, true)).toBe(THEME_DARK);
  });

  it('usa preferência do sistema quando não há escolha salva', () => {
    expect(resolveInitialTheme(null, false)).toBe(THEME_LIGHT);
