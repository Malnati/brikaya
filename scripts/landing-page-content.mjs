...
export const LANDING_LASTMOD = '2026-07-16';
export const PLAY_ROUTE_PATH = '/play/';
export const MIN_LANDING_MAIN_WORDS = 280;

/** Inline recovery for stale SW shells; does not register a service worker. */
export const LANDING_SW_RECOVERY_SCRIPT = `(function(){if(!("serviceWorker"in navigator))return;var play=${JSON.stringify(PLAY_ROUTE_PATH)};var path=location.pathname;var standalone=window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===true;if(standalone&&(path==="/"||path==="/index.html")){location.replace(play);return;}navigator.serviceWorker.getRegistrations().then(function(regs){regs.forEach(function(reg){reg.update();});}).catch(function(){});})();`;
